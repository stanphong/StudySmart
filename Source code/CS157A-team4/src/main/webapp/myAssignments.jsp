...
<title>My Assignments</title><title></title></title></head></html><%@ page language="java" contentType="text/html; charset=UTF-8"
	pageEncoding="UTF-8"%>
<%@ taglib uri="http://java.sun.com/jsp/jstl/core" prefix="c"%>
<!DOCTYPE html>
<html lang="en">
...
<body class="bg-gray-100 font-sans">
	...
	<div class="container mx-auto p-6">
		...
		<h1 class="text-3xl font-bold text-gray-800 mb-6">Manage My
			Assignments For ${param.courseName}</h1>
		...
		<div
			...
			<div
				...
				<div class="modal-content py-4 text-left px-6">
					...
					<!-- Modal Body -->
					<form action="UAddAssignment" method="post" class="space-y-4"
						onsubmit="setTimeout(refreshCourses, 1000)">
						<!-- Assignment Name -->
						<div>
							<label for="assignmentName"
								class="block text-gray-700 text-sm font-bold mb-2">
								Assignment Name: </label> <input type="text" id="assignmentName"
						...
						<div>
							<label for="grade"
								class="block text-gray-700 text-sm font-bold mb-2">
								Grade: </label> <input type="number" id="grade" name="grade" step="0.01"
								required
								class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline"
						...
						<div>
							<label for="maxgrade"
								class="block text-gray-700 text-sm font-bold mb-2">
								Maximum Possible Grade: </label> <input type="number" id="maxgrade"
								name="maxgrade" step="0.01" required
								class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline"
						...
						<div>
							<label for="weight"
								class="block text-gray-700 text-sm font-bold mb-2">
								Weight: </label> <input type="number" id="weight" name="weight"
								step="0.01" required
								class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline"
						...
						<input type="hidden" id="courseId" name="courseId"
							value="${param.courseId}"> <input type="hidden"
							id="courseName" name="courseName" value="${param.courseName}">
		...
		<h2 class="text-2xl font-semibold text-gray-700 mb-4">My
			Assignments</h2>

		<c:if test="${empty assignmentList}">
			<p class="text-gray-600">No Assignments found. Add a new
				assignment to get started!</p>
		</c:if>
						...
						<c:forEach var="assignment" items="${assignmentList}">
							<tr class="border-b border-gray-200 hover:bg-gray-100">
								...
								<td class="py-3 px-6 text-left">
									<form action="UDeleteAssignment" method="post"
										onsubmit="return confirm('Are you sure you want to remove this assignment?') & setTimeout(refreshCourses, 1000);">
										<input type="hidden" name="assignmentId"
											value="${assignment.assignmentId}" /> <input type="hidden"
											id="courseId" name="courseId" value="${param.courseId}">
